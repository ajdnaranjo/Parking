...
namespace Parking.Repositories
{
    public class UserRepository
    {
        ...
        public MonthlyPaymentDto GetMonthlyPaymentByPlate(string plate)
        {
            using (var context = new PLTOEntities())
            {
                return (from mp in context.MonthlyPayments
                        ...
                        where mp.Plate == plate && c.IsActive == true && mp.DeletedDate == null
                        orderby mp.ExpirationDate descending
        ...
        public AppUser GetAppUser(int appUserID)
        {
            using (var context = new PLTOEntities())
            {
                var user = context.AppUsers.FirstOrDefault(x => x.UserID == appUserID );
